...
UserSchema.pre("save", function () {//antes de guardar el modelo de usuario generamos la siguiente funcion
  ...
  bcrypt.genSalt(10, (err, salt) => {//encriptame este password diez veces que es el punto entre seguridad y eficiencia
    if (err) return next(err);//nos devuelve un error
    bcrypt.hash(user.password, salt, null, (err, hash) => {//nos pide cuatro parametros de los cuales nos devolvera un erro o un hash en una promesa
...
UserSchema.methods.matchPassword = async function (password) {//funcion que nos comprueba que el password que pasamos y el de la base de datos segun el usuario coincide
