...
usersCtrl.allUsersActives = async (req, res) => {
  //función para todos los usuarios activos
...
usersCtrl.allUsersInactives = async (req, res) => {
  //funcion todos los usuarios inactivos
  const users = await User.find() //espero que me encuentres a todos los usuarios
...
usersCtrl.allUsersDeleteds = async (req, res) => {
  //todos los usuarios que se dan de baja
...
usersCtrl.userById = async (req, res) => {
  //funcion busqueda por id de usuario
...
usersCtrl.createNewUser = async (req, res) => {
  ...
  const errors = []; //
  if (errors.length > 0) {
    //si los errores son mas de cero, es decir si hay errores dentro de todos los usuarios dentro del array
    /* res.status(400).send({}) {//
      errors,
      title,
      description,
    ...
    //si es lo contrario de la afirmacion anterios, es decir no tenemos errores
